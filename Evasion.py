...
print("\n=================================================")
print("🛡️ DEFENSE AUDIT: Input Denoising (Feature Squeezing)")
print("=================================================")

# 1. Define a pre-processing denoiser (Gaussian Blur)
# This removes the high-frequency adversarial noise generated by the attack
denoiser = GaussianBlur(kernel_size=3, sigma=1.5)

# 2. Apply defense to the adversarial image
denoised_img_tensor = denoiser(masked_img_tensor)

# 3. Evaluate the defended image
with torch.no_grad():
    # Evaluate original adversarial image (Before)
    adv_output = model(masked_img_tensor)
    adv_prob = torch.softmax(adv_output, dim=1)[0][adv_output[0].argmax()].item()
    adv_pred = labels[adv_output[0].argmax()]

    # Evaluate denoised adversarial image (After)
    defended_output = model(denoised_img_tensor)
    defended_prob = torch.softmax(defended_output, dim=1)[0][defended_output[0].argmax()].item()
    defended_pred = labels[defended_output[0].argmax()]

print(f"[Before Defense] Model Prediction: {adv_pred} (Confidence: {adv_prob:.2f}) - ATTACK SUCCESSFUL")
print(f"[After Defense]  Model Prediction: {defended_pred} (Confidence: {defended_prob:.2f}) - ATTACK MITIGATED")

# Optional: Visualize the Before vs After
defended_pil = transforms.functional.to_pil_image(
    unnormalize(denoised_img_tensor).squeeze(0).clamp(0, 1)
)
...
plt.subplot(1, 2, 2)
plt.imshow(defended_pil)
plt.title(f"Denoised Image (Defended)\nPred: {defended_pred}")
plt.axis('off')
plt.show()
